...
function createCacheImplementation<AddEntry>(
  ...
  function commitChangeSet(changeSet: ChangeSet) {
    ...
    // TODO: Wrap this in `unstable_scheduleCallback`
    results.forEach((resultsForResource, resource) => {
  ...
  function accessResult<I, K, V>(
    ...
    const changeSet = readContext(ChangeSetContext, observedBits);
    let result;
    if (changeSet !== null) {
...
export class CacheProvider extends React.Component {
  ...
  componentDidMount() {
    ...
    unstable_scheduleCallback(() => this.setState({cache}));
  }
...
export function useCache() {
  return readContext(CacheContext);
}
