...
describe('ReactCache', () => {
  beforeEach(() => {
    ...
    CacheProvider = ReactCache.CacheProvider;
    createResource = ReactCache.createResource;
    useCache = ReactCache.useCache;
  ...
  it('throws a promise if the requested value is not in the cache', async () => {
    ...
    expect(ReactNoop.flush()).toEqual(['Suspend! [Hi]', 'Loading...']);
  ...
  it('warns if non-primitive key is passed to a resource without a hash function', () => {
    ...
    if (__DEV__) {
      expect(() => {
        expect(ReactNoop.flush()).toEqual(['App', 'Loading...']);
      }).toWarnDev(
        [
  ...
  it('evicts least recently used values', async () => {
    ...
    ReactCache.setGlobalCacheLimit(3);
